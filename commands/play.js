module.exports = {
  ...
  run: async (message, args) => {
    ...
    if (!channel) return message.reply('음성채널에 넣어줘라 시벨럼아');
    if (!args.length) return message.reply('시발 주소 복붙해라');
    ...
    switch (res.loadType) {
      ...
      case 'TRACK_LOADED':
        ...
        return message.reply(`이 시발놈이 틀었네 \`${res.tracks[0].title}\`.`);
      ...
      case 'SEARCH_RESULT':
        ...
        if (first.toLowerCase() === 'end') {
          if (!player.queue.current) player.destroy();
          return message.channel.send('시발 취소했다.');
        }
        ...
        if (!player.playing && !player.paused && !player.queue.size) player.play();
        return message.reply(`이 시발놈이 틀었음 \`${track.title}\`.`);
    }
  },
};
